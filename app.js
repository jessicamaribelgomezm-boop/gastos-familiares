...
const configured =
  window.SUPABASE_URL &&
  !window.SUPABASE_URL.startsWith('PEGA_AQUI') &&
  window.SUPABASE_ANON_KEY &&
  !window.SUPABASE_ANON_KEY.startsWith('PEGA_AQUI');

const db =
  configured && supabaseLib?.createClient
...
function configuredGuard() {
  ...
  if (!db) {
    alert(
      'No se pudo cargar la conexión de Supabase. Actualiza la página e inténtalo de nuevo.'
    );
    return false;
  }
...
function updateAuthUI() {
  ...
  $('authText').textContent =
    authMode === 'signup'
      ? 'Usaremos tu cuenta para sincronizar los gastos entre los dos celulares.'
      : 'Entra con tu cuenta para continuar.';

  $('authName').parentElement.classList.toggle(
    'hidden',
    authMode !== 'signup'
  );
...
async function submitAuth() {
  ...
  if (
    ...
    authMessage(
      'Completa los campos. La contraseña debe tener al menos 6 caracteres.'
    );
    return;
  }
...
async function createHome() {
  ...
  const name =
    ($('homeName').value || 'Nuestro hogar').trim();

  if (!name) {
    alert('Escribe un nombre para el hogar.');
    return;
  }
...
async function joinHome() {
  ...
  try {
    ...
    alert(
      e.message ||
      'No fue posible unirse al hogar.'
    );
  }
}
...
function copyInvite() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard
      ...
      .catch(() => {
        alert(
          'No fue posible copiar automáticamente. Código: ' +
          code
        );
      });
  } else {
...
function enterApp() {
  ...
  if (household) {
    $('homeDisplay').textContent =
      household.name || 'Nuestro hogar';

    $('homeTitle').textContent =
      household.invite_code || 'HOGAR';
  ...
  if (paid) {
    paid.innerHTML =
      members
        .map(
          x =>
            `<option value="${esc(x.user_id)}">${esc(
              x.profiles?.display_name ||
              (
...
$('expenseForm')
  .addEventListener(
    ...
    async e => {
      ...
      if (!household) {
        alert(
          'Primero crea o únete a un hogar.'
        );
        return;
      }
...
$('clearBtn')
  .addEventListener(
    ...
    async () => {
      if (
        !confirm(
          'Esto eliminará todos los gastos del hogar. ¿Continuar?'
        )
      ) {
        return;
      }
